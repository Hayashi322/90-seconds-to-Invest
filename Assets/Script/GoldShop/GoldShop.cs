...
public class GoldShop : MonoBehaviour
{
    ...
    {
        if (!shop) return;
        if (goldbuyValue) goldbuyValue.text = $"Buy:  {shop.BuyGoldPrice.Value:N0}";
        if (goldsellValue) goldsellValue.text = $"Sell: {shop.SellGoldPrice.Value:N0}";
    }

    private void RefreshGold()
    {
        if (!inv) return;
        if (goldAmountValue) goldAmountValue.text = $"Gold Amount: {inv.goldAmount.Value:N0}";
    }
